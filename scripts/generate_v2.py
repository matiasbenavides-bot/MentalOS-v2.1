#!/usr/bin/env python3
"""
MentalOS v2.1 - Interfaz sólida, clara y responsive.
...
# ==================== css/style.css ====================
FILES["css/style.css"] = '''/* MentalOS v2.1 - Clean, responsive, mobile-first */
:root {
  --bg: #0f0f0f;
  --surface: #1c1c1c;
...
/* PIN */
.pin-card {
  text-align: center; padding: 2rem; width: 260px;
}
.pin-card h1 { margin-bottom: 1.5rem; font-size: 2rem; }
#pin-input {
...
.credits-badge {
  display: flex; align-items: center; gap: 0.25rem; font-weight: 600; font-size: 0.9rem;
}
.icon-btn {
...
.bottom-nav {
  ...
  border-top: 1px solid var(--border); padding: 0.4rem 0 0.6rem;
  flex-shrink: 0;
}
.nav-btn {
...
.habit-icon { font-size: 1.8rem; margin-bottom: 0.3rem; }
.habit-name { font-weight: 600; font-size: 0.9rem; }
.habit-value { font-size: 0.75rem; color: var(--text-secondary); margin-top: 0.2rem; }
...
.modal-container {
  ...
  max-width: 500px; max-height: 90vh; overflow-y: auto; padding: 1.5rem 1.2rem 2rem;
...
@media (min-width: 600px) {
  ...
  .modal-container { border-radius: 16px; margin: auto; max-width: 450px; }
}
...
# ==================== js/ui.js (modal, toast, navigation) ====================
...
export function renderExploit() {
  ...
  vp.innerHTML = `
    ...
    ${basal.mode === 'red' ? '<p style="color:var(--red); margin:0.5rem 0;">Modo Protección activo. No podés iniciar sesiones.</p>' : ''}
    <div id="areas-list" style="margin-top:1rem;"></div>
...
export function renderAnalysis() {
  ...
  vp.innerHTML = `
    ...
    <button class="btn-secondary full-width" id="export-btn" style="margin-top:1rem;">📥 Exportar datos</button>
  `;
...
def create_project():
    ...
    for path, content in FILES.items():
        ...
        print(f"  ✓ {path}")
    import shutil
    shutil.copy2(__file__, os.path.join(BASE_DIR, 'scripts', 'generate_v2.py'))
    print(f"\n✅ MentalOS v2.1 generado en '{BASE_DIR}/'")
    print("Abrí index.html en tu navegador.")
